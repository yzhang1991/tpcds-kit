...
strtotime(char *str) {
    ...
    if (hour > 23 || hour < 0) INTERNAL("Invalid time format");
    if (min > 59 || min < 0) INTERNAL("Invalid time format");
    if (sec > 59 || sec < 0) INTERNAL("Invalid time format");
...
strtodate(char *str) {
    ...
    if (sscanf(str, "%d-%d-%d", &res->year, &res->month, &res->day) != 3) INTERNAL(
            "Badly formed string in call to strtodate()");
    res->flags = 0;
...
date_part(date_t *d, int part) {
    switch (part) {
        ...
        case 3:
            return (d->day);
        default: INTERNAL("Invalid call to date_part()");
            return (-1);
    }
}
